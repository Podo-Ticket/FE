...
import {SAPY_GRAYHALL as grayhall} from '@/constants/venue/SAPY';
      ...
      <TransformComponent>
        <SeatMapContainer>
          ...
          <SeatMapContent ref={seatMapRef}>
            <SeatRow>
              {Object.keys(grayhall).map(row => (
                <SeatColumn key={row} seatCount={grayhall[row].length}>
                  {grayhall[row].map(seat => {
