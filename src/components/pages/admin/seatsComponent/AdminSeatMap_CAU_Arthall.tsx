...
import {
  CAU_ARTHALL_A as theater_top_left,
  CAU_ARTHALL_B as theater_top_mid,
  CAU_ARTHALL_C as theater_top_right,
  CAU_ARTHALL_D as theater_bottom_left,
  CAU_ARTHALL_E as theater_bottom_mid,
  CAU_ARTHALL_F as theater_bottom_right,
  CAU_ARTHALL_E_specialSeats as theater_bottom_mid_special,
} from '@/constants/venue/Chung-angUniv';

import {fetchAdminSeats, fetchSeatAudience} from '@/api/admin/RealtimeSeatsApi';
import WheelchairSeat from '@/components/common/buttons/WheelchairSeat';
...
const SeatMap: React.FC<SeatMapProps> = ({
  ...
  return (
    <TransformWrapper
      ...
      wheel={{step: 1}} // 마우스 휠 줌 속도( step: 0.1) (마우스 휠 줌 비활성화 -  disabled: true  )
      ...
      <TransformComponent>
        <SeatMapContainer>
          ...
          <SeatMapContent ref={seatMapRef}>
            <SeatMapTopSide>
              <SeatTopLeft>
                {Object.keys(theater_top_left).map(row => (
                  <SeatColumn key={row} seatCount={theater_top_left[row].length}>
                    {theater_top_left[row].map(seat => {
                      ...
                      return (
                        <SingleSeat
                          key={seatId}
                          isAdmin={true}
                          content={`A ${String(seat).padStart(2, '0')}`}
              ...
              <SeatRow>
                {Object.keys(theater_top_mid).map(row => (
                  <SeatColumn key={row} seatCount={theater_top_mid[row].length}>
                    {theater_top_mid[row].map(seat => {
                      const seatId = `${row}${seat}`;
                      const isReserved = reservedSeats.includes(seatId);
                      const isLocked = lockedSeats.includes(seatId);
                      const isShowSelectedAudience = selectedAudienceSeats.includes(seatId);
                      ...
                      return (
                        <SingleSeat
                          key={seatId}
                          isAdmin={true}
                          content={`B ${String(seat).padStart(2, '0')}`}
                          onClick={() => handleSeatClick(seatId)}
                          isAvailable={isAvailable}
                          isSelectedAudience={isShowSelectedAudience}
                          isReserved={isReserved}
              ...
              <SeatTopRight>
                {Object.keys(theater_top_right).map(row => (
                  <SeatColumn key={row} seatCount={theater_top_right[row].length}>
                    {theater_top_right[row].map(seat => {
                      const seatId = `${row}${seat}`;
                      const isReserved = reservedSeats.includes(seatId);
                      const isLocked = lockedSeats.includes(seatId);
                      const isShowSelectedAudience = selectedAudienceSeats.includes(seatId);
                      ...
                      return (
                        <SingleSeat
                          key={seatId}
                          isAdmin={true}
                          content={`C ${String(seat).padStart(2, '0')}`}
                          onClick={() => handleSeatClick(seatId)}
                          isAvailable={isAvailable}
                          isSelectedAudience={isShowSelectedAudience}
                          isReserved={isReserved}
            ...
            <SeatMapDownSide>
              <SeatRow>
                {Object.keys(theater_bottom_left).map(row => (
                  <SeatColumn key={row} seatCount={theater_bottom_left[row].length}>
                    {theater_bottom_left[row].map(seat => {
                      const seatId = `${row}${seat}`;
                      const isReserved = reservedSeats.includes(seatId);
                      const isLocked = lockedSeats.includes(seatId);
                      const isShowSelectedAudience = selectedAudienceSeats.includes(seatId);
                      ...
                      return (
                        <SingleSeat
                          key={seatId}
                          isAdmin={true}
                          content={`D ${String(seat).padStart(2, '0')}`}
                          onClick={() => handleSeatClick(seatId)}
                          isAvailable={isAvailable}
                          isSelectedAudience={isShowSelectedAudience}
                          isReserved={isReserved}
              ...
              <SeatRow>
                {Object.keys(theater_bottom_mid).map(row => (
                  <SeatColumn key={row} seatCount={theater_bottom_mid[row].length}>
                    {theater_bottom_mid[row].map(seat => {
                      const seatId = `${row}${seat}`;
                      const isReserved = reservedSeats.includes(seatId);
                      const isLocked = lockedSeats.includes(seatId);
                      const isShowSelectedAudience = selectedAudienceSeats.includes(seatId);
                      ...
                      return (
                        <SingleSeat
                          key={seatId}
                          isAdmin={true}
                          content={`E ${String(seat).padStart(2, '0')}`}
                          onClick={() => handleSeatClick(seatId)}
                          isAvailable={isAvailable}
                          isSelectedAudience={isShowSelectedAudience}
                          isReserved={isReserved}
                ...
                <SeatColumn seatCount={8}>
                  {theater_bottom_mid_special.map(({row, number, type}) => {
                    ...
                    const SeatCmp = type === 'wheel' ? WheelchairSeat : SingleSeat;

                    return (
                      <SeatCmp
                        key={seatId}
                        isAdmin={false}
                        content={`E ${String(number).padStart(2, '0')}`}
                        onClick={() => handleSeatClick(seatId)}
                        isAvailable={isAvailable}
                        isSelectedAudience={isShowSelectedAudience}
                        isReserved={isReserved}
              ...
              <SeatRow>
                {Object.keys(theater_bottom_right).map(row => (
                  <SeatColumn key={row} seatCount={theater_bottom_right[row].length}>
                    {theater_bottom_right[row].map(seat => {
                      const seatId = `${row}${seat}`;
                      const isReserved = reservedSeats.includes(seatId);
                      const isLocked = lockedSeats.includes(seatId);
                      const isShowSelectedAudience = selectedAudienceSeats.includes(seatId);
                      ...
                      return (
                        <SingleSeat
                          key={seatId}
                          isAdmin={true}
                          content={`F ${String(seat).padStart(2, '0')}`}
                          onClick={() => handleSeatClick(seatId)}
                          isAvailable={isAvailable}
                          isSelectedAudience={isShowSelectedAudience}
                          isReserved={isReserved}
