...
import {submitEvaluation, submitRecommand} from '../../../../api/user/TicketApi';
          ...
          <StarContent>
            <StarContentHeader>
              <ContentIndex className='Podo-Ticket-Headline-H4'>
                <span className='Podo-Ticket-Headline-H2' style={{color: 'var(--purple-4)'}}>
                  1
                </span>
                /2
              </ContentIndex>
...
const Content = styled.div<{isClosing: boolean; isThanksContent: boolean}>`
  ...
  animation: ${({isClosing, isThanksContent}) =>
      isThanksContent ? (isClosing ? fadeOut : 'none') : isClosing ? fadeOut : fadeIn}
    0.4s ease-in-out;
`;
...
const SliderInput = styled.input.attrs<SliderProps>(() => ({
  ...
  background: linear-gradient(
    ...
    /* 채워진 부분 끝 */ var(--grey-2) ${props => props.value * 10}% /* 비활성화된 부분 시작 */
  );
  border-radius: 13px;
...
const Label = styled.span.attrs<{isActive?: boolean}>(props => ({
  className: props.isActive ? 'Podo-Ticket-Headline-H6' : 'Podo-Ticket-Headline-H6',
}))<{isActive?: boolean}>`
