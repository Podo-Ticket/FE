...
const SingleSeatContainer = styled.button<{
  ...
  border: ${({isLocked, isReserved, isSelected, isSelectedAudience, isLocking, isUnlocking}) => {
  ...
  color: ${({isSelectedAudience, isLocked, isReserved, isSelected, isLocking, isUnlocking}) => {
  ...
  &::before,
  &::after {
    content: ${({isReserved, isLocked, isUnlocking, isSelected}) =>
      isReserved || isLocked || (isSelected && isUnlocking) ? '""' : 'none'}; /* 조건부로 X 표시 */
    ...
    height: 1px; /* 선의 두께 */
    background-color: ${({isUnlocking}) =>
      isUnlocking ? 'var(--purple-4)' : 'var(--ect-white)'}; /* 흰색 선 */
...
const LockImage = styled.div<{isLocked: boolean; isUnlocking: boolean; isAdmin: boolean}>`
  content: ${({isLocked, isUnlocking}) =>
    isLocked || isUnlocking ? '""' : 'none'}; /* 잠금 상태일 때만 표시 */
  position: absolute;
  top: -3px;
  right: -3px;
