...
// 사용자 리스트 가져오기
export const fetchOnsiteUserList = async (
  scheduleId: number,
): Promise<FetchOnsiteUserListResponse> => {
  try {
...
// 승인 요청/거절 API
export const approveOnsite = async (
  userIds: number[],
  scheduleId: number,
  check: boolean,
): Promise<{accept: boolean; error?: string}> => {
  try {
    const response = await api.patch(
      '/reservation/approve',
      {userIds, scheduleId, check},
      {
        withCredentials: true,
      },
    );
